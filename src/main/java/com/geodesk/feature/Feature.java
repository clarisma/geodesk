...
public interface Feature extends Iterable<Feature>
{
	...
	/// If this Feature was returned by a call to [#members()]
	...
	/// Returns the value of a tag as an `int`.
	///
	/// @param key	the key of the tag
	...
	/// Creates a JTS [Geometry] object for this feature. The returned
	/// following types of geometries are created:
    ///
	/// - For a `Node`: [org.locationtech.jts.geom.Point]
	/// - For a non-closed `Way`: [org.locationtech.jts.geom.LineString]
	/// - For a closed `Way`: [org.locationtech.jts.geom.Polygon] if it
	///   represents an area, otherwise [org.locationtech.jts.geom.LinearRing]
	/// - For a `Relation` that is an area: [org.locationtech.jts.geom.Polygon]
	/// - For a non-area `Relation`: [org.locationtech.jts.geom.GeometryCollection]
	///
